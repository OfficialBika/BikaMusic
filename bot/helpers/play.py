...
from bot import app, config, db, logger, queue, yt
...
def checkUB(play):
    async def wrapper(_, m: types.Message):
        ...
        if play_mode or force:
            ...
            if (
                m.from_user.id not in admin_list
        ...
        if chat_id not in db.call_cache:
            ...
            try:
                ...
                if m.chat.username:
                    ...
                    try:
                        ...
                        return await m.reply_text(
                            m.lang["play_invite_error"].format(type(exc).__name__)
                        )

                notice = await m.reply_text(m.lang["play_invite"].format(app.name))
                ...
                try:
                    ...
                    logger.error(f"Error joining chat - {chat_id}: {exc}")
                    return await notice.edit_text(
                        m.lang["play_invite_error"].format(type(exc).__name__)
                    )
